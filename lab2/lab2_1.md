...
```r
top_speed_mean<-(sum(venom_GT, chrion, veyron))/3
top_speed_mean
```
...
This shows that any NAs in data, will affect the type of calculations that can be done with it.

4. Recalculate the mean using the following code chunk. Why is the useful? 
This is useful because it allows us to calculate things even when some data may be missing. 
